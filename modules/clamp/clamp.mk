...
clamp_install_path_implib          :=
clamp_shared_lflags                := -shared
ifeq ($(PLATFORM), WINDOWS)
clamp_install_path_implib          := $(PATH_INSTALL)/lib$(clamp_name_curdir)dll.a
...
clamp_depends_libs_static_path     = $(foreach module_base,$(clamp_depends_modules),$($(module_base)_path_curdir))
clamp_depends_libs_static_src      = $(foreach path,$(clamp_depends_libs_static_path),$(wildcard $(path)*.c))
ifeq ($(PLATFORM), WINDOWS)
clamp_depends_libs_static_src      += $(foreach path,$(clamp_depends_libs_static_path),$(wildcard $(path)platform_specific/windows/*.c))
else ifeq ($(PLATFORM), LINUX)
clamp_depends_libs_static_src      += $(foreach path,$(clamp_depends_libs_static_path),$(wildcard $(path)platform_specific/linux/*.c))
else ifeq ($(PLATFORM), MAC)
clamp_depends_libs_static_src      += $(foreach path,$(clamp_depends_libs_static_path),$(wildcard $(path)platform_specific/mac/*.c))
endif
clamp_depends_libs_static          = $(patsubst %.c, %_static.o, $(clamp_depends_libs_static_src))
...
clamp_clean_files                  += $(clamp_install_path_static) 
...
$(clamp_install_path_static): $(clamp_static_objects)
	ar -rcs $@ $(clamp_static_objects) $(clamp_depends_libs_static)
