...
static uint64_t _type___create_hash(const char* name);
...
static uint64_t _type__create_hash(const char* name) {
    uint64_t result = 0;
...
static type_t* _type__create(const char* abbreviated_name, uint64_t size) {
    ...
    result->hash = _type__create_hash(abbreviated_name);
...
#pragma pack()
    ...
    type_atom_t* t29 = type_atom__create("29", "29", sizeof(s29_t), &_type__print_value_s8);
