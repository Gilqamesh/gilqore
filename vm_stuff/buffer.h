...
# define ALIGNED_BUFFER_PUSH(aligned_buffer, size, alignment) do { \
    ...
    *(aligned_buffer).addr_cur++ = (aligned_buffer).buffer.cur; \
    uint32_t to_grow = (size); \
    uint64_t remainder = ((uint64_t) (aligned_buffer).buffer.cur) % (alignment); \
    *(aligned_buffer).offset_cur++ = remainder; \
    if (remainder) { \
        to_grow += (alignment) - (remainder); \
    } \
    BUFFER_GROW((aligned_buffer).buffer, to_grow); \
} while (false)
# define ALIGNED_BUFFER_POP(aligned_buffer, n) do { \
    ...
    (aligned_buffer).buffer.cur = *((aligned_buffer).addr_cur - (n)); \
    (aligned_buffer).offset_cur -= (n); \
} while (false)
# define ALIGNED_BUFFER_ADDR_AT(aligned_buffer, index, result_addr) do { \
    ASSERT((aligned_buffer).offset_cur - (int32_t) ((index) + 1) >= (aligned_buffer).offset_start && "Aligned buffer underflow."); \
    result_addr = *((aligned_buffer).addr_cur  - (int32_t) ((index) + 1)) + *((aligned_buffer).offset_cur - (int32_t) ((index) + 1)); \
} while (false)
...
typedef struct aligned_buffer {
    ...
    uint32_t* offset_end;
    // start addr of each block
    // todo: change this to start addr of each data, so that ffi_call can directly work with addr_cur
    uint8_t** addr_cur;
    uint8_t** addr_start;
    uint8_t** addr_end;
} aligned_buffer_t;
