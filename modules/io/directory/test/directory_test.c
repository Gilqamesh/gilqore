...
static bool create_test_files(const char* dirpath) {
    ...
    for (u32 i = 0; i < N_OF_TEST_FILES; ++i) {
        TEST_FRAMEWORK_ASSERT(libc__snprintf(buffer, ARRAY_SIZE(buffer), "%s/%s%d", dirpath, "file", i) < (s32) ARRAY_SIZE(buffer));
        struct file file;
        TEST_FRAMEWORK_ASSERT(file__open(&file, buffer, FILE_ACCESS_MODE_READ, FILE_CREATION_MODE_CREATE) == true);
        file__close(&file);
        TEST_FRAMEWORK_ASSERT(libc__snprintf(buffer, ARRAY_SIZE(buffer), "%s/%s%d", dirpath, TESTDIR_NAME, i) < (s32) ARRAY_SIZE(buffer));
