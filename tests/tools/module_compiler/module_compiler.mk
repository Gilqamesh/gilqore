...
module_compiler_test_depends_modules			        := file common time libc file_reader hash compare circular_buffer mod file_writer string directory string_replacer file_path 
...
.PHONY: module_compiler_test_run_all
module_compiler_test_run_all: module_compiler_test_all ## build and run static module_compiler_test
module_compiler_test_run_all: $(module_compiler_test_child_run_targets)
ifneq ($(module_compiler_test_objects),)
module_compiler_test_run_all: $(PATH_INSTALL)/test_framework$(EXT_EXE)
	@$(PATH_INSTALL)/test_framework$(EXT_EXE) $(module_compiler_test_install_path_static)
#	@$(PYTHON) $(PATH_MK_FILES)/pytester.py $(module_compiler_test_install_path_static)
endif

.PHONY: module_compiler_test_run
module_compiler_test_run: module_compiler_test_all
ifneq ($(module_compiler_test_objects),)
module_compiler_test_run: $(PATH_INSTALL)/test_framework$(EXT_EXE)
	@$(PATH_INSTALL)/test_framework$(EXT_EXE) $(module_compiler_test_install_path_static)
#	@$(PYTHON) $(PATH_MK_FILES)/pytester.py $(module_compiler_test_install_path_static)
endif
