    ...
    while (
        ...
        if (token_value_import) {
            ...
            if (*token_value_error_message == '\"') {
                char* enclosing_comment = string__search_n(token_value_error_message + 1, "\"", 1, false);
                if (enclosing_comment) {
                    u32 unique_error_code = module_compiler__get_error_code();
                    file_writer__write_format(
                        &writer, error_codes_file,
                        "%u REMOVE_ME %.*s\n",
                        unique_error_code,
                        enclosing_comment + 1 - token_value_error_message, token_value_error_message
                    );
                    file_writer__write_format(
                        &writer, &preprocessed_file,
