...
static token_t scanner__make_number(scanner_t* self, bool seen_dot) {
    ...
    if (seen_dot) {
        if (scanner__eat_if(self, 'f') || scanner__eat_if(self, 'F')) {
            return scanner__make_token(self, TOKEN_R32_LITERAL);
        } else {
            return scanner__make_token(self, TOKEN_R64_LITERAL);
        }
    } else {
        if (scanner__eat_if(self, 'u') || scanner__eat_if(self, 'U')) {
            return scanner__make_token(self, TOKEN_UINT_LITERAL);
        } else {
            return scanner__make_token(self, TOKEN_SINT_LITERAL);
        }
    }
}
...
static token_type_t scanner__identifier_type(scanner_t* self) {
    switch (*self->start) {
        ...
        case 'o': return scanner__identifier_type_helper(self, 1, "r",     TOKEN_PIPE);
...
void scanner__init(scanner_t* self, const char* source, uint64_t source_len, bool source_is_mutable) {
    ...
    self->source_is_mutable = source_is_mutable;
}
...
const char* token_type__to_str(token_type_t type) {
    ...
    case TOKEN_SINT_LITERAL: return "SINT_LITERAL";
    case TOKEN_UINT_LITERAL: return "UINT_LITERAL";
    case TOKEN_R32_LITERAL: return "R32_LITERAL";
    case TOKEN_R64_LITERAL: return "R64_LITERAL";
...
token_t scanner__scan_token(scanner_t* self) {
    ...
    switch (c) {
        ...
        case '^': return scanner__make_token(self, TOKEN_HAT);
        case '~': return scanner__make_token(self, TOKEN_BITWISE_NOT);
        case '|': return scanner__make_token(self, scanner__eat_if(self, '|') ? TOKEN_LOGICAL_OR    : TOKEN_PIPE);
        case '&': return scanner__make_token(self, scanner__eat_if(self, '&') ? TOKEN_LOGICAL_AND   : TOKEN_AND);
