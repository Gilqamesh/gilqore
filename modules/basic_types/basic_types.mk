...
basic_types_install_path_implib          :=
basic_types_shared_lflags                := -shared
ifeq ($(PLATFORM), WINDOWS)
basic_types_install_path_implib          := $(PATH_INSTALL)/lib$(basic_types_name_curdir)dll.a
...
basic_types_depends_libs_static_path     = $(foreach module_base,$(basic_types_depends_modules),$($(module_base)_path_curdir))
basic_types_depends_libs_static_src      = $(foreach path,$(basic_types_depends_libs_static_path),$(wildcard $(path)*.c))
ifeq ($(PLATFORM), WINDOWS)
basic_types_depends_libs_static_src      += $(foreach path,$(basic_types_depends_libs_static_path),$(wildcard $(path)platform_specific/windows/*.c))
else ifeq ($(PLATFORM), LINUX)
basic_types_depends_libs_static_src      += $(foreach path,$(basic_types_depends_libs_static_path),$(wildcard $(path)platform_specific/linux/*.c))
else ifeq ($(PLATFORM), MAC)
basic_types_depends_libs_static_src      += $(foreach path,$(basic_types_depends_libs_static_path),$(wildcard $(path)platform_specific/mac/*.c))
endif
basic_types_depends_libs_static          = $(patsubst %.c, %_static.o, $(basic_types_depends_libs_static_src))
...
basic_types_clean_files                  += $(basic_types_install_path_static) 
...
$(basic_types_install_path_static): $(basic_types_static_objects)
	ar -rcs $@ $(basic_types_static_objects) $(basic_types_depends_libs_static)
