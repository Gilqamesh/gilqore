...
void console__log(struct console* self, char* msg, ...)
{
    if (self->out_handle != INVALID_HANDLE_VALUE) {
        ...
        i32 number_of_characters_written = vsnprintf(self->buffer, self->buffer_size, msg, ap);
...
void console__fatal(struct console* self, char *error_msg, ...)
{
    if (self->err_handle != INVALID_HANDLE_VALUE) {
        ...
        i32 number_of_characters_written = vsnprintf(self->buffer, self->buffer_size, error_msg, ap);
        if (number_of_characters_written < 0) {
            // todo(david): diagnostic, error
            ExitProcess(APP_CONSOLE_FATAL);
        }
